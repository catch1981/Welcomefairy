...
try:
    ...
    print(f"[BOOT] Failed to import firebase_admin: {e}")
    firebase_admin = None
    credentials = None
    firestore = None
...
WELCOME_CHANNEL_ID = os.getenv("WELCOME_CHANNEL_ID", "0")
WITCHPATH_CHANNEL_ID = os.getenv("WITCHPATH_CHANNEL_ID", "0")
FRACTUREPATH_CHANNEL_ID = os.getenv("FRACTUREPATH_CHANNEL_ID", "0")
...
def to_int(name, val, default=0):
    try:
        return int(str(val).strip())
    except Exception:
        log.warning(f"ENV {name}='{val}' is not an int. Using {default}.")
        return default

GUILD_ID = to_int("GUILD_ID", GUILD_ID, 0)
WELCOME_CHANNEL_ID = to_int("WELCOME_CHANNEL_ID", WELCOME_CHANNEL_ID, 0)
WITCHPATH_CHANNEL_ID = to_int("WITCHPATH_CHANNEL_ID", WITCHPATH_CHANNEL_ID, 0)
FRACTUREPATH_CHANNEL_ID = to_int("FRACTUREPATH_CHANNEL_ID", FRACTUREPATH_CHANNEL_ID, 0)

log.info("Boot env check → "
...
if not WELCOME_CHANNEL_ID:
    log.error("Missing WELCOME_CHANNEL_ID (channel to post the welcome panel).")
    raise SystemExit(1)
...
if FIREBASE_SERVICE_ACCOUNT and firebase_admin and credentials and firestore:
    ...
    log.warning("FIREBASE_SERVICE_ACCOUNT not set or firebase_admin missing; Firebase logging disabled.")
...
async def ensure_role(guild: discord.Guild, name: str) -> discord.Role:
    ...
    if role is None:
        try:
            ...
            log.error("Missing permission to create roles. Grant the bot 'Manage Roles'.")
            raise
    return role
...
async def log_choice(member: discord.Member, choice: str):
    ...
    try:
        ...
        log.info(f"Logged choice to Firestore: user={member.id} choice={choice}")
    except Exception as e:
...
class PathChoiceView(discord.ui.View):
    ...
    async def _pick(self, itx: discord.Interaction, choice: str):
        ...
        try:
            ...
            return await itx.response.send_message("I need **Manage Roles** permission to assign roles.", ephemeral=True)
...
if __name__ == "__main__":
    log.info("Starting bot run loop...")
